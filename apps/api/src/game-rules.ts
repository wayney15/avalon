...
export function validateRoleSelection(
  ...
  if (selectedRoles.length < 3 || selectedRoles.length > playerCount) {
    return {
      message: `Provide between 4 and ${playerCount} roles. Merlin, Percival, Assassin, and Morgana are always required.`,
      ok: false
    };
  }
  ...
  if ((counts.get("morgana") ?? 0) !== 1) {
    return { message: "Morgana is required exactly once.", ok: false };
  }
...
function visiblePlayersForPlayer(
  ...
  revealedDisconnectedUserIds: Set<string>
): ViewerVisiblePlayer[] {
  const visiblePlayers: ViewerVisiblePlayer[] = [];

  const pushVisiblePlayer = (player: ViewerVisiblePlayer): void => {
    const existingIndex = visiblePlayers.findIndex((entry) => entry.userId === player.userId);
    if (existingIndex >= 0) {
      visiblePlayers[existingIndex] = player;
      return;
    }
  ...
  switch (viewer.role) {
    case "merlin":
      for (const assignment of assignments) {
        if (assignment.userId === viewer.userId || !isKnownToMerlin(assignment.role)) {
          continue;
        }
    ...
    case "percival":
      for (const assignment of assignments) {
        if (
          ...
          (assignment.role !== "merlin" && assignment.role !== "morgana")
        ) {
          continue;
        }
  ...
  for (const assignment of assignments) {
    if (assignment.userId === viewer.userId || !revealedDisconnectedUserIds.has(assignment.userId)) {
      continue;
    }
...
export function buildViewerSecretState(
  ...
  const revealedDisconnectedUserIdSet = new Set(revealedDisconnectedUserIds);
  ...
  return {
    ...
    team: viewer.team,
    viewerRole: "player",
    visiblePlayers: visiblePlayersForPlayer(viewer, assignments, revealedDisconnectedUserIdSet)
  };
}
